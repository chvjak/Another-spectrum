...
const [wasmPath, snapshotPath, refreshText = "1200"] = process.argv.slice(2);
if (!wasmPath || !snapshotPath) {
  throw new Error("usage: capture_gameplay_controls.mjs core.wasm snapshot.sna [refreshes]");
}
...
function rgbFrame() {
  ...
  for (let y = 0; y < 24; ++y) for (let x = 0; x < 160; ++x) {
    const color = frame[input++]; pixel(color); pixel(color);
  }
  for (let y = 0; y < 192; ++y) {
    for (let x = 0; x < 16; ++x) { const color = frame[input++]; pixel(color); pixel(color); }
    for (let x = 0; x < 32; ++x) {
      let bits = frame[input++];
      const attribute = frame[input++];
      const ink = ((attribute & 0x40) >> 3) | (attribute & 7);
      const paper = (attribute & 0x78) >> 3;
      for (let bit = 0; bit < 8; ++bit) { pixel(bits & 0x80 ? ink : paper); bits = (bits << 1) & 0xff; }
    }
    for (let x = 0; x < 16; ++x) { const color = frame[input++]; pixel(color); pixel(color); }
  }
  for (let y = 0; y < 24; ++y) for (let x = 0; x < 160; ++x) {
...
const setKeys = (...keys) => {
  for (const key of [[5, 1], [5, 2], [7, 1]]) core.keyUp(...key);
  for (const key of keys) core.keyDown(...key);
};

let framesWritten = 0;
for (let refresh = 1; refresh <= refreshes; ++refresh) {
  if (refresh === 201) setKeys([5, 1]);       // P: run right
  if (refresh === 601) setKeys([7, 1]);       // SPACE: fire
  if (refresh === 641) setKeys([5, 1], [7, 1]);
  if (refresh === 801) setKeys([5, 2]);       // O: run left
  if (refresh === 1101) setKeys();
  const status = core.runFrame();
  if (status !== 0) throw new Error(`emulator status ${status} at refresh ${refresh}`);
  if ((refresh & 1) !== 0) continue;
  if (!process.stdout.write(rgbFrame())) await once(process.stdout, "drain");
  framesWritten++;
}
...
process.stderr.write(`${JSON.stringify({
  refreshes,
  framesWritten,
  presentations: memory[fixed(0x9f04)] | (memory[fixed(0x9f05)] << 8),
  action: u8(0x9f10),
