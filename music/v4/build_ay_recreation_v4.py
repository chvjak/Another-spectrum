...
def add_beat(st,en,intensity):
    ...
    while t<en:
        ...
        # very sparse metallic tick, gives the original tracker-like machine pulse
        if i%16==12: drums.append(Drum(sec(t+BEAT*.48),'metal',0.22*intensity))
        i+=1; t+=BEAT
...
def bass_env(age,dur,peak):
    # Bowed/machine-like: short bite, settle, slow breathing pulse.
    if age<3: x=[0.45,0.92,1.0][age]
    elif age<9: x=[0.86,0.76,0.70,0.67,0.65,0.64][age-3]
    else:
        x=0.62 + 0.07*math.sin(2*math.pi*age/41.0) + 0.035*math.sin(2*math.pi*age/13.0)
    rem=dur-age
...
def machine_strength(tick):
    s=tick/FPS; out=0.0
    for a,b,k in machine_windows:
        if a<=s<b:
            edge=min(s-a,b-s,2.5)/2.5
            out=max(out,k*max(0,min(1,edge)))
    return out
...
for t in range(TICKS):
    ...
    e=bass_at[t]
    if e:
        age=t-e.start; dur=e.duration; ms=machine_strength(t)
        # sampled-bass bite: octave flick at note start, then subtle fifth/octave grain
        semitone=0
        ...
        r[7]&=0xFE
        # more continual noise modulation, but low enough not to swallow pitch
    ...
    if active_d:
        ...
        lens={'snare':13,'brush':18,'thump':10,'metal':9}; L=lens[kind]
        if age>=L: active_d=None
            ...
            if kind=='snare':
                ...
                p=period(76-round(4*x))
                r[4],r[5]=p&255,(p>>8)&15
                r[6]=2+round(5*x)
                env=[5,6,5,4,3,2,2,1,0][age]
                r[10]=min(15,round(env*s*1.7)); r[7]&=0xDB
...
(OUT/'aw_intro_ay_v4_2m50s.bin').write_bytes(stream)

with (OUT/'aw_intro_ay_v4_registers.csv').open('w',newline='') as f:
...
(OUT/'aw_intro_ay_v4_2m50s.vgm').write_bytes(vgm)
with gzip.open(OUT/'aw_intro_ay_v4_2m50s.vgz','wb',compresslevel=9) as f:f.write(vgm)
...
def post(x,peak=.84):
    ...
    sos=butter(2,[35,9500],btype='bandpass',fs=SR,output='sos'); y=sosfilt(sos,x)
    m=np.max(np.abs(y));
...
mixwav=OUT/'another_world_intro_ay_v4_noisier_envelopes_2m50s.wav'; wav(mixwav,mix)
names=['stem_A_bass_machine_noise','stem_B_melody_sampled_envelopes','stem_C_gentle_snares']
...
mp3=OUT/'another_world_intro_ay_v4_noisier_envelopes_2m50s.mp3'; flac=OUT/'another_world_intro_ay_v4_noisier_envelopes_2m50s.flac'
...
summary={'duration_seconds':DURATION,'ticks_50hz':TICKS,'delta_stream_bytes':len(stream),'vgm_bytes':len(vgm),'changes':['more continuous machine-noise gating on A','noise transients on melody attacks','sample-like pitch macros on note attacks','two-stage sampled-style melody volume envelopes','body+noise gentle snares','sparse metallic machine ticks'],'melody_events':len(melody),'bass_events':len(bass),'drum_events':len(drums)}
(OUT/'summary.json').write_text(json.dumps(summary,indent=2))
readme='''# Another World-inspired AY music v4 — noisier / sampled-envelope pass\n\nDuration: 2:50. 50 Hz AY register timing.\n\nChanges from v3:\n- bass is roughened with low-level AY noise gating and short octave/fifth attack macros\n- melody attacks include brief noise and harmonic pitch macros\n- melody volume macros use fast sample-like decay into a quieter sustain, rather than generic ADSR\n- snares combine a short descending tone body with a longer noise tail\n- sparse metallic ticks and changing noise periods make the machine ambience less static\n\nThe supplied project note says the prior corrected pass used the original tracker notes, periods and rhythm, but it does not contain the original sample waveforms/envelope tables. Therefore these envelopes are a closer AY approximation, not a sample-exact reconstruction.\n'''
(OUT/'README.md').write_text(readme)
shutil.copy2(Path(__file__).resolve(),OUT/'build_ay_recreation_v4.py')

bundle=OUT/'another_world_intro_ay_v4_bundle.zip'
include=['README.md','summary.json','build_ay_recreation_v4.py',mp3.name,flac.name,*[f'{n}.mp3' for n in names],'aw_intro_ay_v4_2m50s.bin','aw_intro_ay_v4_2m50s.vgm','aw_intro_ay_v4_2m50s.vgz','aw_intro_ay_v4_registers.csv']
